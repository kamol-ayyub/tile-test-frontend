...
export function CartTable() {
  return (
    <section>
      <h2 className="font-display text-lg">SHOPPING CART & DESIGN TOOL</h2>

      <div className="mt-3 overflow-hidden rounded-sm border-2 border-navy">
        <table className="w-full border-collapse">
            ...
            <tr>
              {COLUMNS.map((column) => (
                <th
                  key={column}
                  scope="col"
                  className="border border-navy bg-tan px-1.5 py-1 text-[10px] font-bold leading-tight whitespace-pre-line"
                >
                  {column}
                </th>
              ))}
          ...
          <tbody>
            {CART_ITEMS.map((item) => {
              ...
              return (
                <tr key={item.id}>
                  <td className="border border-navy p-1.5">
                    <div className="flex flex-col items-center gap-1">
                      <Icon className="size-11 rounded-sm border border-navy" />
                      <span className="text-center text-[9px] font-bold leading-none">
                        {item.name}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="border border-navy p-1.5 text-center">
                    <span className="inline-block min-w-12 rounded-sm  py-1 text-xs font-bold whitespace-nowrap tabular-nums">
                      [ {item.quantity} ]
                    </span>
                  </td>
                  <td className="border border-navy p-1.5 text-center">
                    <span className="inline-block min-w-14 rounded-sm  py-1 text-xs font-bold whitespace-nowrap tabular-nums">
